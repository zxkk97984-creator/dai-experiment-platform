...
- 重复入队只产生一条有效任务
- Redis 不可用时任务保留在 DB pending，由恢复扫描重新入队
"""
...
# 可重试的前置状态（仅这些状态可以转为 queued）
RETRYABLE_STATUSES = ["pending", "system_error"]
...
def enqueue_job(db: Session, *, job_type: str, object_id: int) -> bool:
    """唯一入队入口：条件 UPDATE 将 pending/system_error 转为 queued，然后推送 Redis。
    ...
    返回：
        ...
        False — 条件更新未命中（已是 queued/running/completed，重复入队被忽略）

    竞争安全：
        使用 UPDATE ... WHERE grading_status IN ('pending', 'system_error') 实现原子抢占。
    ...
    if result.rowcount == 0:
        # 未命中：状态不是 pending/system_error（可能已经 queued/running/completed）
        db.commit()  # 确保事务结束
        return False
...
def requeue_stale_jobs(db: Session, *, job_type: str | None = None,
    ...
    """恢复扫描：处理卡住的 pending / queued / running 任务。
