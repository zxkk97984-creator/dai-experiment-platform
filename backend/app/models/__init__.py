...
class User(TimestampMixin, Base):
    ...
    # TASK-012：全会话撤销版本——改密/管理员重置/禁用时原子 +1；
    # 所有 token 携带 sv，认证/刷新时与库值比对，不一致（含旧 token 无 sv）即 401
...
class AcademicTerm(TimestampMixin, Base):
    ...
    # TASK-010：显式使用迁移中的索引名（uq_academic_terms_code），避免 autogenerate 重命名
    __table_args__ = (Index("uq_academic_terms_code", "code", unique=True),)
...
class Assignment(TimestampMixin, Base):
    ...
    # TASK-010：迁移 B 已在实库收紧为 NOT NULL（回填后），模型对齐。
...
class Submission(TimestampMixin, Base):
    ...
    # TASK-010：显式声明迁移 c3d4e5f6a708 中的复合索引（grading_status, updated_at），
    # 覆盖恢复扫描查询；禁止 autogenerate 将其删除
    __table_args__ = (Index("ix_submissions_gs_updated", "grading_status", "updated_at"),)
...
class NotebookTemplateVersion(Base):
    ...
    # TASK-010：published_at 保持 nullable（与实库一致）
