...
<template>
  <AppLayout>
    <main class="page">
      ...
      <section class="data-panel"><div class="filter-bar"><label class="search-control"><AppIcon name="search" :size="18" /><input v-model="query" placeholder="搜索课程名称" /></label><select v-model="statusFilter" aria-label="状态筛选"><option value="all">状态：全部</option><option value="published">已发布</option><option value="draft">草稿</option><option value="archived">已归档</option></select><select v-model="termFilter" aria-label="学期筛选"><option value="all">学期：全部学期</option><option v-for="term in terms" :key="term" :value="term">{{ term }}</option></select><select v-model="sortOrder" aria-label="排序"><option value="updated">排序：最近更新</option><option value="title">排序：课程名称</option></select></div><div v-if="loading" class="loading-list"><span v-for="i in 6" :key="i" class="skeleton"></span></div><div v-else-if="filteredCourses.length === 0" class="empty-state"><p>📚 暂无符合条件的课程</p></div><div v-else class="table-scroll"><table><thead><tr><th>课程名称</th><th>所属学期 / 班级</th><th>状态</th><th>章节 / 课时</th><th>学生人数</th><th>最近更新</th><th>操作</th></tr></thead><tbody><tr v-for="course in filteredCourses" :key="course.id"><td><a class="course-link" @click="router.push(`/teacher/courses/${course.id}/manage`)">{{ course.title }}</a><small>{{ course.description || '课程教学内容与安排' }}</small></td><td>{{ course.term || course.semester || course.academic_term || '—' }}<small>{{ course.class_name || course.classroom || '未设置班级' }}</small></td><td><span class="status-pill" :class="courseStatus(course)">{{ courseLabel(courseStatus(course)) }}</span></td><td>{{ course.chapter_count ?? course.chapters_count ?? '—' }} 章 / {{ course.lesson_count ?? course.lessons_count ?? '—' }} 课时</td><td>♙ {{ course.student_count ?? course.students_count ?? 0 }}</td><td class="muted-cell">{{ formatDate(courseUpdated(course)) }}</td><td class="actions-cell"><button class="text-action" @click="router.push(`/teacher/courses/${course.id}/manage`)">课程设置</button><button class="text-action" @click="router.push(`/teacher/courses/${course.id}/manage`)">章节课时</button><button v-if="course.status === 'draft'" class="publish-action" @click="handlePublish(course)">发布</button></td></tr></tbody></table></div><footer v-if="!loading && filteredCourses.length" class="pagination-bar"><span>共 {{ filteredCourses.length }} 条</span><span>10 条/页　‹　<span class="active-page">1</span>　2　›</span></footer></section>
    </main>
  </AppLayout>
</template>
