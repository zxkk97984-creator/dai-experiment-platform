...
async function load() {
  const [termRes, classRes] = await Promise.all([
    academicsAPI.listTerms({ page_size: 100 }), academicsAPI.listClasses({ page_size: 100 }),
  ])
  terms.value = termRes.data.items || []
...
async function selectClass(item) { selectedClassId.value = item.id; candidates.value = []; await loadRoster() }
async function loadRoster() {
  const res = await academicsAPI.listClassStudents(selectedClassId.value, { page_size: 100 })
...
<template>
  <AppLayout><main class="academic-page">
    ...
    <section v-if="selectedClass" class="panel roster"><div class="roster-title"><div><h2>{{ selectedClass.name }} · 班级名单</h2><p>{{ termName(selectedClass.academic_term_id) }}</p></div><span>{{ classStudents.length }} 人</span></div>
      ...
      <table><thead><tr><th>姓名</th><th>学号</th><th>账号</th><th></th></tr></thead><tbody><tr v-for="student in classStudents" :key="student.id"><td>{{ student.real_name }}</td><td>{{ student.student_no }}</td><td>{{ student.username }}</td><td><button @click="removeStudent(student)">移出</button></td></tr></tbody></table>
    </section>
  </main></AppLayout>
</template>

<style scoped>
.academic-page{display:grid;gap:20px}header,.panel{border:1px solid var(--border);border-radius:14px;background:var(--surface);box-shadow:var(--shadow-card)}header{display:flex;justify-content:space-between;align-items:center;padding:24px 28px;background:linear-gradient(135deg,#f5f9ff,#fff)}header p{margin:0;color:var(--primary);font-size:11px;font-weight:700;letter-spacing:.12em}header h1{margin:4px 0;font-size:30px}header span,small{color:var(--text-secondary)}.grid{display:grid;grid-template-columns:1fr 1fr;gap:18px}.panel{padding:20px}.panel h2{margin:0 0 14px}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.form-grid button{min-height:42px}.rows{display:grid;gap:8px;margin-top:16px}.rows>div{display:grid;grid-template-columns:1fr auto auto;align-items:center;gap:10px;padding:12px;border:1px solid var(--border);border-radius:9px;cursor:pointer}.rows>div.selected{border-color:var(--primary);background:var(--primary-light)}.rows span{display:grid;gap:3px}.rows em{padding:3px 8px;border-radius:999px;background:#eef2f7;font-size:11px;font-style:normal}.rows em.active{color:#078758;background:#e8f8f0}.rows button,table button{border:0;background:transparent;color:var(--danger)}.roster-title,.search{display:flex;align-items:center;justify-content:space-between;gap:12px}.roster-title h2,.roster-title p{margin:0}.roster-title>span{padding:5px 12px;border-radius:999px;background:var(--primary-light);color:var(--primary)}.search{margin:18px 0}.search input{flex:1}.candidates{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px}.candidates button{padding:8px 10px;border:1px solid var(--border);border-radius:8px;background:#f8fafc}.candidates b{margin-left:8px;color:var(--primary)}table{width:100%;border-collapse:collapse}th,td{padding:11px;border-bottom:1px solid var(--border);text-align:left}@media(max-width:900px){.grid{grid-template-columns:1fr}.form-grid{grid-template-columns:1fr}}
</style>
