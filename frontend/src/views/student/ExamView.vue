        ...
        <article v-for="(question, index) in questions" :key="question.id" class="review-card">
          ...
          <div v-if="visibility.answers" class="standard-answer">
            ...
            <ul v-else-if="question.question_type === 'fill_blank'">
              <li v-for="blank in blankAnswers(question)" :key="blank.id"><code>{{ blank.id }}</code><span>{{ blank.accepted.join(' / ') || '未配置' }}</span></li>
            </ul>
          ...
          <div v-if="visibility.answers && savedAnswerMap.get(question.id)?.manual_score_reason" class="teacher-score-note">
            <span>教师改分说明</span>
