// AppIcon 语义键 → 本地 Iconify 图标数据（Remix/Logos）。
// 全部图标为本地打包数据，运行时不依赖 Iconify API。
// 禁止用 emoji、自制 SVG 或 CSS 图案代替真实图标。
//
// TASK-025（F-29）：单一映射源——契约测试直接枚举本模块的键，
// 新增/遗漏图标都会在测试中暴露（未知键生产保持安全 fallback）。
...
export const ICONS = {
  ...
  // TASK-025（F-29）：课程锁定课时使用——此前缺失导致警告且锁定状态无图标
  lock: { data: lock, set: 'ri' },
}
