...
# ── 1. 构建镜像 ──────────────────────────────────────────────
# VERIFY_BUILD_EXTRA_ARGS 用于本地环境定制（如 "VERIFY_BUILD_EXTRA_ARGS='--network=host'
# ./scripts/verify_build_context.sh" 让构建步骤经宿主机网络访问本地代理）；CI 保持默认。
read -r -a BUILD_EXTRA_ARGS <<< "${VERIFY_BUILD_EXTRA_ARGS:-}"
log "构建 backend 镜像 ..."
docker build "${BUILD_EXTRA_ARGS[@]}" -t "$BACKEND_IMG" backend/

log "构建 frontend 镜像 ..."
docker build "${BUILD_EXTRA_ARGS[@]}" -t "$FRONTEND_IMG" frontend/
...
if [ -f backend/.env ]; then
    ...
    while IFS= read -r value; do
        [ -n "$value" ] || continue
        # 跳过无敏感性的本地开发地址与镜像源（连接串默认值与代码内默认一致属预期）
        case "$value" in
            http://localhost*|http://127.0.0.1*|redis://localhost*|mysql+pymysql://*@localhost*) continue ;;
        esac
        scanned=$((scanned + 1))
...
# nginx.conf 的 upstream "api" 仅在 Compose 网络内解析；独立烟测时映射到回环
# 使 nginx 能启动（只验证静态首页，不验证 API 反代——API 反代由 Compose 烟测覆盖）。
cid="$(docker run -d --add-host api:127.0.0.1 -p "$FRONTEND_PORT":80 "$FRONTEND_IMG")"
